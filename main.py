...
# Optional authorization — for search without login
...
def trigger_ingestion(
    ...
    # Duplicate fix — check for active job for this URL
    # Validace schedule
    ...
    if not source:
        ...
        # Update permission_type if changed
        source.permission_type = request.permission_type
        db.commit()

    # Create job in advance so ingestion can find it
...
async def search_rag(
    ...
    user: Optional[UserInfo] = Depends(get_optional_user)  # Works without login
):
    try:
